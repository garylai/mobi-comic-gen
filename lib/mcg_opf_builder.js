...
class MCGOPFBuilder {
    ...
    _makeMOBIAsync() {
        ...
        const mobiFileName = this._title + '.mobi';
        return new Promise(function(res, rej) {
            executeFile(binaryPath, [opfFilePath, '-c0', '-o', mobiFileName], (error, stdout, stderr) => {
                if(error != null) {
                    rej(error);
                }
                res(path.join(this._bookRoot, mobiFileName));
            });
        }.bind(this));
    }
