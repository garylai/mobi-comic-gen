...
class MCGComicManager {
    constructor(comicId) {
        ...
        this._getPageInfoScriptRegex = new RegExp(util.format('function dm5imagefun\\(\\){var cid=%d;var key=\'(\\w+)\';var pix="(http[\\w:./-]+)";var pvalue=(\\[.+\\]);for\\(var i=0;i<pvalue\\.length;i\\+\\+\\){pvalue\\[i\\]=pix\\+pvalue\\[i\\]\\+\'\\?cid=%d&key=.+\'}return pvalue}var d;d=dm5imagefun\\(\\);', comicId, comicId));
    }

    getPageImagesAsync() {
        return co(function* (){
            yield this._loadCookiesAsync();
            const imageUrls =  yield this._getPageInfoAsync();
            return yield this._downloadImagesToTmpFolderAsync(imageUrls);
        }.bind(this));
    }
    ...
    _downloadImagesToTmpFolderAsync(imageUrls) {
        return co(function* (){
            ...
            for(var i = 0; i < imageUrls.length; i++) {
                const targetPath = path.resolve(tmpFolder, './' + uuid.v4());
                const promise = new Promise(function(res, rej) {
                    const stream = request({
                        ...
                        jar: this._cookieJar,
                        gzip: true,
                        headers: this._getImageHeader(imageUrls[i]),
                        timeout: 10000
